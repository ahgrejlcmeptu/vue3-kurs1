...
const App = {
  ...
  methods: {
    prev() {
      this.activeIndex = this.activeIndex === this.steps.length ? 0 : this.activeIndex - 1
    },
    nextOfFinish() {
      this.activeIndex++
    },
  ...
  computed: {
    activeStep() {
      let step = this.activeIndex > this.steps.length - 1 ? this.steps.length - 1 : this.activeIndex
      return step
    },
    lastStap() {
      let text = this.activeIndex < this.steps.length - 1 ? 'Вперед' : 'Закончить'
      return text
    },
    resetBtn() {
      let text = this.activeIndex < this.steps.length ? 'Назад' : 'Начать заново'
      return text
    }
  }
}
